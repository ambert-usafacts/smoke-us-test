...
<script>
	...
	function findItem(evt) {
		...
		// Get the array of domain values and the range
		...
		// Calculate which band the mouse is over
		const index = Math.abs(Math.floor((range[1] - mouseX) / bandWidth));

		// Reverse the index to get the correct value from domainValues
		const reversedIndex = domainValues.length - 1 - index;

		// Make sure the index is within the domain's array bounds
		if (reversedIndex >= 0 && reversedIndex < domainValues.length) {
			const closestXValue = domainValues[reversedIndex];

			// Find the data point corresponding to closestXValue
			found = $data.find((d) => d.name === closestXValue);
